"""Simulation Engine for MiroFish - Runs parallel multi-agent simulations."""
...
class SimulationEngine:
    ...
    async def _run_round(
        ...
        """Run a single simulation round with all personas in parallel."""
        interactions = []
        events = []
        ...
        async def _run_persona(persona: Persona) -> dict:
            try:
                response = await self._get_persona_response(persona, round_context, state.config)
                return {
                ...
                logger.error(f"Persona {persona.name} failed in round {round_num}: {e}")
                return {
                    "persona_id": persona.id,
                    "persona_name": persona.name,
                    "persona_role": persona.role.value,
        ...
        tasks = [_run_persona(p) for p in state.personas]
        results = await asyncio.gather(*tasks, return_exceptions=True)

        for result in results:
            if isinstance(result, dict):
                interactions.append(result)
                # Extract key events from responses
                if "error" not in result.get("response", "").lower():
                    events.append(f"{result['persona_name']} ({result['persona_role']}): {result['response'][:100]}")
    ...
    async def _get_persona_response(
        ...
        """Get a single persona's response for the current round."""
        memory_str = "\n".join(f"- {m}" for m in persona.memory[-5:]) if persona.memory else "No prior context."

        messages = [
            {"role": "system", "content": persona.system_prompt},
            {"role": "system", "content": f"Your identity: {persona.name}, Role: {persona.role.value}\nTraits: {', '.join(persona.traits)}\nGoals: {', '.join(persona.goals)}"},
            {"role": "system", "content": f"Your memory/context:\n{memory_str}"},
            {"role": "user", "content": f"{round_context}\n\nBased on your role and perspective, what happens next? How do you react? What actions do you take? Be specific and realistic."},
        ]
        ...
        try:
            ...
            logger.error(f"LLM call failed for persona {persona.name}: {e}")
            try:
                response, _ = await llm_router.invoke_with_fallback("moderator", messages)
                return getattr(response, "content", str(response))
            except Exception as e2:
                return f"Response unavailable: {e2}"
